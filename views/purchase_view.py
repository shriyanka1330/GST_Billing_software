...
from models.voucher import create_purchase_voucher

class PurchaseView:
    def __init__(self, parent):
        self.base = BaseForm(parent, "Purchase Voucher")
        ...
        tk.Label(top, text="Supplier Name").grid(row=0, column=2, padx=10)
        ...
        tk.Label(top, text="Purchase Type").grid(row=1, column=0, pady=5)
        ...
        tk.Button(btn_f, text="Save Purchase Voucher", command=self.save_voucher, bg="#4CAF50", fg="white", width=20).pack(side="right")
    ...
    def save_voucher(self):
        ...
        if not ledger_name:
            messagebox.showerror("Error", "Select a Supplier Ledger")
            return
        if not self.cart:
        ...
        try:
            ...
            messagebox.showinfo("Success", f"Purchase Voucher {v_no} generated successfully!")
